...
const ChartContainor = ({ selectedUser, setSelectedUser }) => {
    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messagesDummyData.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== "680f50e4f10f3cd28382ecf9" && 'flex-row-reverse'}`}>
            {msg.image ? (
              <img src={msg.image} alt="" className={`max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-4 ${msg.senderId === "680f50e4f10f3cd28382ecf9" ? ' rounded-br-none' : ' rounded-bl-none'}`} />
            ) : (
              <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-4 break-all bg-violet-500/30 text-white ${msg.senderId === "680f50e4f10f3cd28382ecf9" ? ' rounded-br-none' : ' rounded-bl-none'}`}>{msg.text}</p>
            )}
      ...
      <div className=' absolute bottom-0 right-0 left-0 flex items-center gap-3 p-3'>
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
          ...
          <input type="file" className='' id='image' accept='image/png , image/jpeg' hidden />
          <label htmlFor="image">
        ...
        <span className='w-10 h-10 cursor-pointer bg-violet-500 flex items-center justify-center rounded-full'>
