...
const ChartContainor = () => {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative bg-white/2'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-gray-600/20'>
        ...
        <span className='max-md:hidden max-w-5'>
          <InfoCircleOutlined onClick={() => setSelectedUser(false)} style={{ color: "#e5e7eb", fontSize: "22px" }} />
        </span>
      </div>
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser.id && 'flex-row-reverse'}`}>
            {msg.image ? (
              ...
              <p className={`p-3 max-w-[200px] md:text-sm font-light rounded-lg mb-4 break-all shadow-md ${
